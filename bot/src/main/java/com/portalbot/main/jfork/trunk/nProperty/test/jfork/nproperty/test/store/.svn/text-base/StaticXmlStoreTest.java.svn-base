...
@Cfg
public class StaticXmlStoreTest
{
	@Cfg
	...
	{
		try (BufferedWriter writer = new BufferedWriter(new FileWriter("config/stored.xml")))
		{
			ConfigParser.storeXml(StaticXmlStoreTest.class, writer);
		}

		ConfigParser.parseXml(Subclass.class, "config/stored.xml");

		for (Field field : Subclass.class.getDeclaredFields())
		{
			field.setAccessible(true);
			Object object1 = field.get(Subclass.class);
			Object object2 = StaticXmlStoreTest.class.getDeclaredField(field.getName()).get(StaticXmlStoreTest.class);
